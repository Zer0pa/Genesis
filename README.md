...
**Honest blocker:** the experiment is live, not frozen. Phase 0, Phase 1, Phase 2, Phase 2.5, and the full current Phase 3 prep at-scale receipt surface are now in the repo: 11 BIG cells plus the first parity-sweep receipt at S20. Three of the four pre-registered comparisons now have receipt-backed verdicts; the D6-vs-C3 symmetry comparison remains PENDING because the Z2-asymmetric observable has not yet been implemented. K2-task parity is confirmed at S30 and widened operationally by new parity-sweep cells, but the broader multi-step host-side comparison surface is not yet complete. Treat the repo as a live window into the work, with future receipts appended rather than rewritten.
...
| [`proofs/artifacts/sigma_curve_full.tsv`](proofs/artifacts/sigma_curve_full.tsv) | 73-line aggregated Phase 2 + 2.5 + complete Phase 3 prep + initial parity-sweep σ″ curve table |
...
- **K2-task parity sweep extension** — `Active Engineering`. `BITDET_K2_S20_PARITY` is now in-repo with RM10 target SHA `74fa0b8a…`; `S40_PARITY` and `S50_PARITY` remain live-manifest scope until their receipts are pulled and host-side byte-compared.
...
- **Cross-platform parity at K2-task level** — `CONFIRMED at S30`, `Active Engineering` for broader coverage. `k2_summary.json` is byte-identical between M1 host and RM10 at `--steps 30`; `BITDET_K2_S20_PARITY` now provides the RM10 comparison target `74fa0b8a…`, and extending parity confirmation to additional step values remains a smaller host-side comparison task.
